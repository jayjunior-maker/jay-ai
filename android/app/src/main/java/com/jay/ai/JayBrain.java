...
public class JayBrain {
    ...
    public String think(String input) {
        ...
        if (containsAny(text, "what can you do", "what can u do", "what do you do", "explain what you can do",
                "what are your capabilities", "unaweza kufanya nini"))
            return "Sir, I can work locally with supported phone actions, local memory and conversation features. I can open installed apps, check time, battery and device information, open the camera, and call contacts when Android permissions are granted. For broader questions I can use Jay's online brain when it is available.";
        ...
        if (containsAny(text, "are you online", "are you still online", "online status", "upo online"))
        ...
        if (containsAny(text, "what did i tell you", "what did i ask", "unakumbuka nini")) {
            String memory = database.getMemory("last_request");
            return memory == null ? "I don't have anything saved in that memory yet, Sir." : "I remember you said: " + memory;
        }
