...
public final class JayLocalCommandManager {
    ...
    private boolean isDateCommand(String text) {
        return text.equals("today") || text.equals("what is today") || text.equals("when is today") || text.equals("date") || text.equals("what date is it") || text.equals("leo ni lini") || text.equals("leo ni tarehe gani") || text.contains("today's date") || text.contains("what day is today");
    }
    ...
    private String extractBackgroundColor(String text) {
        Matcher hex = Pattern.compile("#[0-9a-f]{6}|#[0-9a-f]{8}", Pattern.CASE_INSENSITIVE).matcher(text);
        if (hex.find()) return hex.group();
        String[] colors = {"black", "blue", "purple", "red", "green", "cyan", "dark", "default"};
        for (String c : colors) if (text.contains(c)) return c;
        return null;
    }
    ...
    private String getLastKnownLocation() {
        ...
        try {
            ...
            if (manager == null) return "Location services are unavailable, Sir.";
            Location best = null;
            String[] providers = {LocationManager.GPS_PROVIDER, LocationManager.NETWORK_PROVIDER, LocationManager.PASSIVE_PROVIDER};
            for (String provider : providers) {
                try { Location l = manager.getLastKnownLocation(provider); if (l != null && (best == null || l.getTime() > best.getTime())) best = l; } catch (SecurityException ignored) { }
            }
            if (best == null) return "I don't have a recent location fix yet, Sir.";
            return String.format(Locale.US, "Your last known location is latitude %.6f, longitude %.6f, Sir.", best.getLatitude(), best.getLongitude());
        } catch (Exception e) { return "I couldn't read your location, Sir."; }
    }

    private String whatsappMessage(String original) {
        if (!appManager.isAppInstalled("WhatsApp")) return "WhatsApp isn't installed on this phone, Sir.";
        String lower = original.toLowerCase(Locale.ROOT);
        int marker = lower.indexOf("text ");
        if (marker < 0) marker = lower.indexOf("message ");
        if (marker < 0) marker = lower.indexOf("send ");
        if (marker < 0) return "Who should I message on WhatsApp, Sir?";
        String rest = original.substring(marker + (lower.startsWith("text ", marker) ? 5 : lower.startsWith("message ", marker) ? 8 : 5)).trim();
        String[] parts = rest.split("\\s+", 2);
        if (parts.length < 2) return "What message should I send, Sir?";
        String contact = parts[0];
        String message = parts[1];
        String found = contactManager.findPhoneNumber(contact);
        if ("PERMISSION_REQUIRED".equals(found)) return "Sir, Jay needs Contacts permission before it can find that WhatsApp contact.";
        if (!found.startsWith("CONTACT_FOUND|")) return "I couldn't find a contact named " + contact + ", Sir.";
        String[] data = found.split("\\|", 3);
        if (data.length < 3) return "I couldn't read that contact's phone number, Sir.";
        String number = data[2].replaceAll("[^0-9+]", "");
        try {
            String digits = number.startsWith("+") ? number.substring(1) : number;
            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://wa.me/" + digits + "?text=" + Uri.encode(message)));
            intent.setPackage("com.whatsapp");
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
            return "Opening WhatsApp with a message to " + data[1] + ", Sir. Please confirm/send it in WhatsApp.";
        } catch (Exception e) { return "I couldn't open that WhatsApp conversation, Sir."; }
    }
