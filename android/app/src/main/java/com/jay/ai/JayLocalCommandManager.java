...
public final class JayLocalCommandManager {
    ...
    public String handle(String input) {
        ...
        if (containsAny(text, "battery", "bata", "beteri", "battery status", "battery health")) {
            return JayDeviceTools.getBattery(context) + ".";
        }

        if (containsAny(text, "phone information", "phone info", "device information", "device info",
                "information about my phone", "about my phone")) {
        ...
        if (text.startsWith("call ") || text.startsWith("piga simu ") || text.startsWith("pigia ")) {
            String name = text.startsWith("call ") ? input.trim().substring(5).trim()
                    : text.startsWith("piga simu ") ? input.trim().substring(10).trim()
        ...
        if (text.startsWith("open ")) {
            ...
            if (!appName.isEmpty() && !appName.equalsIgnoreCase("settings")
                    && !appName.equalsIgnoreCase("camera") && !appName.equalsIgnoreCase("phone")
                    && !appName.equalsIgnoreCase("calendar")) {
