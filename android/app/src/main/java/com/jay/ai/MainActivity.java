...
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private void startFirstLaunchPermissionSetup() {
        if (getSharedPreferences("jay_setup", MODE_PRIVATE).getBoolean("completed", false)) return;
    ...
    private TextView makeLabel(String text,float size){TextView v=new TextView(this);v.setText(text);v.setTextColor(Color.WHITE);v.setTextSize(size);return v;}
    private Button makeAction(String text){Button b=new Button(this);b.setText(text);return b;}
    private void buildJayInterface(){
        rootView=new LinearLayout(this);rootView.setOrientation(LinearLayout.VERTICAL);rootView.setPadding(20,18,20,14);applySavedAppearance();
        LinearLayout header=new LinearLayout(this);header.setGravity(Gravity.CENTER_VERTICAL);TextView title=makeLabel("JAY",30);title.setTypeface(null,1);header.addView(title,new LinearLayout.LayoutParams(0,-2,1));Button settings=makeAction("⚙");settings.setOnClickListener(v->showSettings());header.addView(settings,new LinearLayout.LayoutParams(60,60));rootView.addView(header);
        jayStatus=makeLabel("● JAY ONLINE",14);jayStatus.setGravity(Gravity.CENTER);jayStatus.setPadding(0,0,0,8);rootView.addView(jayStatus);
        TextView core=makeLabel("J",78);core.setGravity(Gravity.CENTER);GradientDrawable ring=new GradientDrawable();ring.setShape(GradientDrawable.OVAL);ring.setColor(Color.rgb(12,18,42));ring.setStroke(5,Color.rgb(80,170,255));core.setBackground(ring);LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(210,210);cp.gravity=Gravity.CENTER;rootView.addView(core,cp);
        TextView mode=makeLabel("PERSONAL AI  •  LOCAL-FIRST",12);mode.setGravity(Gravity.CENTER);rootView.addView(mode);
        ScrollView scroll=new ScrollView(this);conversation=makeLabel("Jay: "+jayConversationManager.getStartupGreeting(isNetworkAvailable())+"\n\n",16);conversation.setPadding(16,12,16,12);scroll.addView(conversation);rootView.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));
        ...
        LinearLayout actions=new LinearLayout(this);actions.setGravity(Gravity.CENTER);Button talk=makeAction("🎙 TALK");talk.setOnClickListener(v->startListening());Button send=makeAction("SEND");send.setOnClickListener(v->{String m=inputBox.getText().toString().trim();if(!m.isEmpty()){processMessage(m);inputBox.setText("");}});actions.addView(talk,new LinearLayout.LayoutParams(0,-2,1));actions.addView(send,new LinearLayout.LayoutParams(0,-2,1));rootView.addView(actions);setContentView(rootView);
    }
    ...
    private boolean handleAppearanceCommand(String message){String lower=message.toLowerCase(Locale.ROOT).trim();if(lower.contains("change wallpaper")||lower.contains("change the wallpaper")||lower.equals("wallpaper")){try{Intent i=new Intent(Intent.ACTION_SET_WALLPAPER);startActivity(Intent.createChooser(i,"Choose wallpaper"));reply("Opening the wallpaper selector, Sir.");}catch(Exception e){reply("Sir, I couldn't open the wallpaper selector.");}return true;}String[]colors={"black","blue","purple","red","green","cyan","dark","default"};for(String color:colors)if(lower.contains("background")&&lower.contains(color)){if(JayAppearanceManager.setBackgroundColor(this,color)){applySavedAppearance();reply("Background changed to "+color+", Sir.");}else reply("Sir, I couldn't change the background to "+color+".");return true;}if(lower.contains("background")&&lower.contains("#")){int h=lower.indexOf('#');String value=lower.substring(h).split("\\s")[0];if(JayAppearanceManager.setBackgroundColor(this,value)){applySavedAppearance();reply("Background color changed, Sir.");}else reply("Sir, that background color isn't valid.");return true;}if(lower.contains("restore")&&lower.contains("background")){JayAppearanceManager.reset(this);applySavedAppearance();reply("Default Jay background restored, Sir.");return true;}return false;}
