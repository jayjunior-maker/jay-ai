...
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private void add(LinearLayout p,android.view.View v,int l,int t,int r,int b){p.addView(v);android.widget.LinearLayout.LayoutParams x=(android.widget.LinearLayout.LayoutParams)v.getLayoutParams();x.setMargins(l,t,r,b);v.setLayoutParams(x);}
    ...
    private void buildJayInterface(){
        ...
        LinearLayout header=new LinearLayout(this);header.setGravity(Gravity.CENTER_VERTICAL);Button menu=makeAction("☰");menu.setTextSize(24);menu.setBackground(panel(Color.rgb(20,45,80),Color.rgb(7,16,32),22));header.addView(menu,new LinearLayout.LayoutParams(58,58));LinearLayout brand=new LinearLayout(this);brand.setOrientation(LinearLayout.VERTICAL);TextView title=makeLabel("JAY",40,Color.WHITE);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);TextView sub=makeLabel("AI ASSISTANT",16,BLUE);brand.addView(title);brand.addView(sub);header.addView(brand,new LinearLayout.LayoutParams(0,68,1));Button settings=makeAction("⚙");settings.setTextSize(28);settings.setBackground(panel(Color.rgb(30,85,145),Color.rgb(5,14,30),28));settings.setOnClickListener(v->showSettings());header.addView(settings,new LinearLayout.LayoutParams(70,70));add(content,header,0,4,0,12);
        ...
        LinearLayout inputRow=new LinearLayout(this);inputRow.setGravity(Gravity.CENTER_VERTICAL);inputRow.setPadding(14,4,7,4);inputRow.setBackground(panel(BLUE,Color.rgb(5,17,37),45));inputBox=new EditText(this);inputBox.setHint("Ask Jay anything...");inputBox.setHintTextColor(Color.rgb(135,155,185));inputBox.setTextColor(Color.WHITE);inputBox.setTextSize(18);inputBox.setSingleLine(true);inputRow.addView(inputBox,new LinearLayout.LayoutParams(0,66,1));Button mic=makeAction("🎙");mic.setTextSize(23);mic.setBackground(panel(0,Color.rgb(8,25,49),20));mic.setOnClickListener(v->startListening());inputRow.addView(mic,new LinearLayout.LayoutParams(62,60));add(content,inputRow,8,0,8,14);
        ...
        LinearLayout cards=new LinearLayout(this);cards.setGravity(Gravity.CENTER);cards.addView(feature("💬","CHAT","With Jay",Color.rgb(190,110,255)),new LinearLayout.LayoutParams(0,145,1));cards.addView(feature("🧠","MEMORY","Jay's Memory",Color.rgb(70,255,110)),new LinearLayout.LayoutParams(0,145,1));cards.addView(feature("◇","TASKS","Automations",Color.rgb(255,175,30)),new LinearLayout.LayoutParams(0,145,1));cards.addView(feature("☎","PHONE","Assistant",Color.rgb(255,95,95)),new LinearLayout.LayoutParams(0,145,1));cards.addView(feature("▦","APPS","Applications",Color.CYAN),new LinearLayout.LayoutParams(0,145,1));add(content,cards,0,0,0,12);

        LinearLayout service=new LinearLayout(this);service.setGravity(Gravity.CENTER_VERTICAL);service.setPadding(14,8,14,8);service.setBackground(panel(Color.rgb(20,50,90),Color.rgb(6,16,33),26));service.addView(makeLabel("🛡",27,Color.rgb(0,255,170)),new LinearLayout.LayoutParams(50,56));LinearLayout ss=new LinearLayout(this);ss.setOrientation(LinearLayout.VERTICAL);ss.addView(makeLabel("Background services are active",15,Color.WHITE));ss.addView(makeLabel("Jay is working in the background",12,Color.LTGRAY));service.addView(ss,new LinearLayout.LayoutParams(0,60,1));service.addView(makeLabel("MANAGE  ›",14,BLUE),new LinearLayout.LayoutParams(100,60));add(content,service,0,0,0,12);
        ...
        LinearLayout nav=new LinearLayout(this);nav.setGravity(Gravity.CENTER);nav.setPadding(4,4,4,4);nav.setBackground(panel(Color.rgb(20,55,100),Color.rgb(6,16,34),30));nav.addView(navButton("⌂\nHome",true),new LinearLayout.LayoutParams(0,62,1));nav.addView(navButton("◌\nTalk",false),new LinearLayout.LayoutParams(0,62,1));nav.addView(navButton("▣\nMemory",false),new LinearLayout.LayoutParams(0,62,1));Button ns=navButton("⚙\nSettings",false);ns.setOnClickListener(v->showSettings());nav.addView(ns,new LinearLayout.LayoutParams(0,62,1));add(content,nav,0,0,0,2);
        page.addView(content);rootView.addView(page,new LinearLayout.LayoutParams(-1,0,1));setContentView(rootView);
    }
    private LinearLayout feature(String icon,String title,String sub,int accent){LinearLayout b=new LinearLayout(this);b.setOrientation(LinearLayout.VERTICAL);b.setGravity(Gravity.CENTER);b.setPadding(4,5,4,4);b.setBackground(panel(Color.rgb(25,55,100),Color.rgb(7,15,32),24));TextView i=makeLabel(icon,24,accent);i.setGravity(Gravity.CENTER);b.addView(i,new LinearLayout.LayoutParams(-1,36));TextView t=makeLabel(title,14,Color.WHITE);t.setGravity(Gravity.CENTER);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.addView(t,new LinearLayout.LayoutParams(-1,27));TextView s=makeLabel(sub,10,Color.LTGRAY);s.setGravity(Gravity.CENTER);b.addView(s,new LinearLayout.LayoutParams(-1,24));return b;}
