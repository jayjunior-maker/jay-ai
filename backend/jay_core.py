...
Jay's API does not require a paid model key. It combines deterministic local
capabilities with public web retrieval. External model providers can be added
later behind the same interface without changing the Android client.
"""
    ...
    try:
        ...
        def evaluate(node):
            ...
            if isinstance(node, ast.BinOp) and type(node.op) in _OPERATORS:
                ...
                if isinstance(node.op, ast.Pow) and abs(right) > 100: raise ValueError("power too large")
...
def local_answer(message: str) -> Optional[str]:
    """Handle useful requests without a paid AI API."""
    ...
    if lower in {"help", "what can you do"} or "what can you do" in lower:
        return ("I can chat with you, use the internet for current information, "
                "and work with Jay's memory, security and device tools as those "
                "capabilities are connected, Sir.")
    expression = None
    if lower.startswith("calculate "): expression = q[10:].strip()
    elif lower.startswith("what is ") and re.fullmatch(r"[0-9+\-*/().%\s]+", q[8:].strip()): expression = q[8:].strip()
    if expression:
        value = _safe_math(expression)
        if value is not None: return f"The answer is {value}, Sir."
    return None


def build_online_answer(message: str, web_search: Callable, article: Callable) -> tuple[str, str]:
    """Answer from public web sources without an external AI API."""
    results = web_search(message)
    if not results:
        return ("I couldn't find reliable information about that online, Sir. I also don't want to invent an answer.", "web")
    best_title = results[0].get("title", "")
    if best_title:
        data = article(best_title)
        if data and data.get("extract"):
            extract = re.sub(r"\s+", " ", data["extract"]).strip()
            if len(extract) > 3500: extract = extract[:3500].rsplit(" ", 1)[0] + "..."
            return (f"Here is what I found online, Sir.\n\n{data.get('title', best_title)}\n\n{extract}", "web")
    parts = []
    for result in results[:5]:
        title = re.sub(r"\s+", " ", result.get("title", "")).strip()
        snippet = re.sub(r"\s+", " ", result.get("snippet", "")).strip()
        if title and snippet: parts.append(f"{title}\n{snippet}")
    if parts: return "I found these online results, Sir.\n\n" + "\n\n".join(parts), "web"
